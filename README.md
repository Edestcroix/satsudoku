...
- `-S --summarize` can only be used with `-a`. Outputs a summary of the benchmarking results to `[output]/summary.txt`. This will contain the averages of CPU time, propagations, and propagation rates for each encoding, for both easy and hard puzzles.
